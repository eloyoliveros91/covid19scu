...
muestras = pd.read_excel(DATA_PATH.joinpath("COVID-19.xlsx"), sheet_name = 'Relacion de muestras')
confirmadosOriente = pd.read_excel(DATA_PATH.joinpath("COVID-19.xlsx"), sheet_name = 'Oriente', skiprows=[0], usecols="A:G")
ingresos = pd.read_excel(DATA_PATH.joinpath("COVID-19.xlsx"), sheet_name = 'Ingresos SCU')
...
trace2 = go.Bar(
    x=muestras['Fecha'], y=muestras['Muestras Confirmadas'],
    name='Muestras Confirmadas'
)
trace3 = go.Bar(
    x=muestras['Fecha'], y=muestras['Muestras Negativas'],
    name='Muestras Negativas'
)
trace4 = go.Bar(
    x=muestras['Fecha'], y=muestras['Muestras Confirmada por 2da Vez'],
    name='Muestras Confirmada por 2da Vez'
)

hombresIngresos = go.Bar(
    x=ingresos['Fecha'], y=ingresos['Hombres'],
    name='Hombres'
)
mujeresIngresos = go.Bar(
    x=ingresos['Fecha'], y=ingresos['Mujeres'],
    name='Mujeres'
)
adultosIngresos = go.Bar(
    x=ingresos['Fecha'], y=ingresos['Adultos'],
    name='Adultos'
)
ninosIngresos = go.Bar(
    x=ingresos['Fecha'], y=ingresos['Niños'],
    name='Niños'
)
cubanoIngresos = go.Bar(
    x=ingresos['Fecha'], y=ingresos['Cubano'],
    name='Cubano'
)
extranjeroIngresos = go.Bar(
    x=ingresos['Fecha'], y=ingresos['Extranjero'],
    name='Extranjero'
)

data = [trace3, trace2, trace4]
layout = go.Layout(
    barmode='stack',
    xaxis=dict(tickvals=muestras['Fecha'])
)
muestrasFigure = go.Figure(data=data, layout=layout)
sexoFig = go.Figure(data=[hombresIngresos, mujeresIngresos], layout=go.Layout(
    barmode='stack',
    xaxis=dict(tickvals=ingresos['Fecha'])
))
adultosFig = go.Figure(data=[adultosIngresos, ninosIngresos], layout=go.Layout(
    barmode='stack',
    xaxis=dict(tickvals=ingresos['Fecha'])
))
cubanosFig = go.Figure(data=[cubanoIngresos, extranjeroIngresos], layout=go.Layout(
    barmode='stack',
    xaxis=dict(tickvals=ingresos['Fecha'])
))
...
app.layout = html.Div(
    [
        ...
        html.Div(
            [
                    ...
                    [
                        html.Div(
                            [
                                ...
                                html.H5(
                                    "SANTIAGO DE CUBA", style={"margin-top": "0px"}
                                ),
                            ]
                        )
                    ],
                ...
                html.Div(
                    [
                        html.P([
                            html.B("Fecha : " + str(dt.date.today()))
                        ],
        ...
        html.Div(
            [
                #cajas de textos confirmados, activos, etc...
                html.Div([
                    html.Div(
                        [html.H6(id="confirmados_text"), html.P("Casos Confirmados"),
                        html.P(
                            html.B(ultimoDia[3])
                    ...
                    html.Div(
                        [html.H6(id="activos_Text"), html.P("Casos Activos"),
                        html.P(
                            html.B(ultimoDia[5])
                        , style={'textAlign' : 'center', 'font-size':'30px'})],
                        id="activos",
                    ...
                    html.Div(
                        [html.H6(id="altas_Text"), html.P("Altas médicas"),
                        html.P(
                            html.B(ultimoDia[4])
                        , style={'textAlign' : 'center', 'font-size':'30px'})],
                        id="altas",
                    ...
                    html.Div(
                        [html.H6(id="fallecidos_Text"), html.P("Fallecidos"),
                        html.P(
                            html.B(ultimoDia[6])
                        , style={'textAlign' : 'center', 'font-size':'30px'})],
                        id="fallecidos",
            ...
            className="row flex-display cuadros-texto",
        ),
        html.Div(
            [
                html.Div(
                    dcc.Graph(
                        ...
                        figure={
                            ...
                            {
                                'x': casosConfirmados['Fecha'],
                                'y': casosConfirmados['Nuevos Casos Confirmados'],
                                'name': 'Nuevos Casos Confirmados'
                            }],
                            'layout': {                            
                            'plot_bgcolor': '#f9f9f9'#rgb(249, 249, 249),                
                            } 
                        },
                        config={
                    ...
                    id='confirmadosContainer',
                    className='pretty_container seven columns'
                ),
                html.Div(
                    [dcc.Graph(id="muestrasSantiago",
                    figure=muestrasFigure), ],
        ...
        html.Div(
            [
                html.Div(
                    [dcc.Graph(
                        id='confirmados_oriente',
                        figure={
                            'data': [{
                                'x': confirmadosOriente['Fecha'],
                                'y': confirmadosOriente['Las Tunas'],
                                'name': 'Las Tunas'
                            },
                            {
                                'x': confirmadosOriente['Fecha'],
                                'y': confirmadosOriente['Holguín'],
                                'name': 'Holguín'
                            },
                            {
                                'x': confirmadosOriente['Fecha'],
                                'y': confirmadosOriente['Granma'],
                                'name': 'Granma'
                            },
                            {
                                'x': confirmadosOriente['Fecha'],
                                'y': confirmadosOriente['Santiago de Cuba'],
                                'name': 'Santiago de Cuba'
                            },
                            {
                                'x': confirmadosOriente['Fecha'],
                                'y': confirmadosOriente['Guantánamo'],
                                'name': 'Guantánamo'
                            }]
                        },        
                    ),],
